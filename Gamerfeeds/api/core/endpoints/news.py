...
from api.core.schemas import NewsSchema
...
def get_all_news(db: Session = Depends(get_db)):
    all_news = db.scalar(select(News).all())
    if not all_news:
        raise HTTPException(
...
@router.post('/news', status_code=status.HTTP_201_CREATED, response_model=NewsSchema)
